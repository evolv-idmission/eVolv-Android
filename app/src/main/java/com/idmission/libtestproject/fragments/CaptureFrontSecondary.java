...
public class CaptureFrontSecondary extends Fragment implements ImageProcessingResponseListener {
...
//                    fragmentManager.beginTransaction().replace(R.id.flContent, processFlow).addToBackStack(null).commit();
    ...
    private void captureCameraAPICall() {
        ImageProcessingSDK.getInstance().setImageProcessingResponseListener(CaptureFrontSecondary.this);
...
//                ImageProcessingSDK.getInstance().autoCapture(getActivity(), ImageType.FRONT, null, titleBitmap, 0,  null, commonFunctioObject);
                ...
                ImageProcessingSDK.getInstance().autoCapture(getActivity(), ImageType.FRONT_SECONDARY, commonFunctioObject, null, null);
    ...
    public void onResume() {
        ...
        Bitmap bitmap = BitmapUtils.getBitmapFromFile(eVolvApp.getBaseDirectoryPath() + Constants.FRONT_IMAGE_SECONDARY);
        setImage(bitmap);
        setSavedValue();
    }
    ...
    public void onAutoImageCaptureResultAvailable(Map<String, String> resultMap, Response response) {
        ...
        if (null != response) {
            if (response.getStatusCode() == ResponseStatusCode.PERMISSION_NOT_GRANTED.getStatusCode()) {
                ...
                if (null != resultMap) {
                    if (resultMap.containsKey(ImageType.FRONT_SECONDARY.toString())) {
                        imageBase64 = resultMap.get(ImageType.FRONT_SECONDARY.toString());
                        if (!StringUtil.isEmpty(imageBase64)) {
                            byte[] decodedString = Base64.decode(imageBase64, Base64.DEFAULT);
                            Bitmap bitmap = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);

                            BitmapUtils.writeBitmapToFilepath(eVolvApp.getBaseDirectoryPath() + Constants.FRONT_IMAGE_SECONDARY, bitmap);
